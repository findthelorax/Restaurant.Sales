...
export const useHandleSubmissionError = (selectedTeamMember) => {
    ...
    const handleSubmissionError = (error, dailyTotals) => {
        if (error.response && error.response.status === 400) {
            alert(
                `Totals on ${dailyTotals.date} for ${selectedTeamMember.teamMemberFirstName} ${selectedTeamMember.teamMemberLastName} - ${selectedTeamMember.position} already exists.`
            );
        } else {
