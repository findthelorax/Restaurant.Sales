...
export const signupStyles = (theme) => {
	const spacing = theme && typeof theme.spacing === 'function' ? theme.spacing : () => 0;
	const breakpoints = theme ? theme.breakpoints : { up: () => {} };
	...
	return {
		main: {
			...
			display: 'block',
			width: 'auto',
			...(breakpoints && breakpoints.up
				? {
						[breakpoints.up(400 + spacing(2))]: {
							width: 400,
							marginLeft: 'auto',
							marginRight: 'auto',
						},
